...
class EditOutputWidgetImpl : public EditOutputWidget
{
    ...
    QLineEdit* name_ = 0;
    QCheckBox* useDelay_ = 0;
    QCheckBox* preserveDelay_ = 0;
    QSpinBox*  delaySec_ = 0;
...
public:
	...
	sub_layout->setColumnStretch(2, 0);
	sub_layout->setColumnStretch(3, 1);
	sub_layout->setColumnStretch(4, 0);
	sub_layout->setColumnStretch(5, 1);
	sub_layout->addWidget(new QLabel(obs_module_text("StreamDelay"),
		this), 0, 0);
	sub_layout->addWidget(useDelay_ = new QCheckBox(nullptr,this),0,1);
	sub_layout->addWidget(new QLabel(obs_module_text("StreamPreserve"),
		this), 0, 2);
	sub_layout->addWidget(preserveDelay_ = new QCheckBox(nullptr, this), 0,
		3);
		sub_layout->addWidget(new QLabel(obs_module_text("DelaySec"),
			this), 0, 4);

		delaySec_ = new QSpinBox(this);
		delaySec_->setMinimumSize(QSize(80, 0));
		delaySec_->setSuffix(QString::fromUtf8(" s"));
		delaySec_->setMinimum(0);
		delaySec_->setMaximum(1800);
		layout->addWidget(delaySec_, currow,
			5);
		sub_layout->addWidget(delaySec_, 0,5);

		delaySec_->setEnabled(true);
		preserveDelay_->setEnabled(true);
		useDelay_->setEnabled(true);

		QObject::connect(useDelay_, &QPushButton::toggled, [this]() {
			preserveDelay_->setEnabled(!useDelay_->isChecked());
			delaySec_->setEnabled(!useDelay_->isChecked());
		});
	...
	conf_["useDelay"] = useDelay_->isChecked();
	conf_["preserveDelay"] = preserveDelay_->isChecked();
	conf_["delaySec"] = delaySec_->value();
	...
	it = conf_.find("useDelay");
	if (it != conf_.end() && it->isBool()) {
		useDelay_->setChecked(it->toBool());
		if (it->toBool()) {
			preserveDelay_->setEnabled(false);
			delaySec_->setEnabled(false);
		}
	}

	it = conf_.find("preserveDelay");
	if (it != conf_.end() && it->isBool())
		preserveDelay_->setChecked(it->toBool());

	it = conf_.find("delaySec");
	if (it != conf_.end() && it->isDouble())
		delaySec_->setValue(it->toInt());
