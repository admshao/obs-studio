...
class PushWidgetImpl : public PushWidget, public IOBSOutputEventHanlder
{
	...
	obs_data_set_bool(conf, "useDelay", conf_["useDelay"].toBool());
	obs_data_set_bool(conf, "preserveDelay", conf_["preserveDelay"].toBool());
	obs_data_set_int(conf, "delaySec", conf_["delaySec"].toInt());

	bool useObsDelay = conf_["useDelay"].toBool();
	bool obsDelay = config_get_bool(obs_frontend_get_profile_config(), "Output", "DelayEnable");
	int obsDelaySec = config_get_int(obs_frontend_get_profile_config(), "Output", "DelaySec");
	bool obsPreserveDelay = config_get_bool(obs_frontend_get_profile_config(), "Output", "DelayPreserve");

	if (useObsDelay) {
		if (obsDelay)
			obs_output_set_delay(output_, obsDelaySec,
					     obsPreserveDelay
						     ? OBS_OUTPUT_DELAY_PRESERVE
						     : 0);
	}
	else
		obs_output_set_delay(output_, conf_["delaySec"].toInt(), conf_["preserveDelay"].toBool() ? OBS_OUTPUT_DELAY_PRESERVE : 0);
